...
def main():
    ...
    cursor=conn.cursor()
    sql="select album_id,album_name from albuminfo"
    ...
    # 歌曲id号 歌曲名字
        ...
        url = 'https://music.163.com/weapi/v1/resource/comments/R_AL_3_' + str(albumid) + '?csrf_token='
        data = {'params': params, 'encSecKey': encSecKey}
        # url = 'https://music.163.com/#/song?id=19292984'
        # 获取第一页评论
        html = get_comments_json(url, data)
        if html==0:
            print("专辑",str(albumid)," ",str(albumname),"爬取失败")
            with open('getalbumcmt_log.txt','a+',encoding='utf8') as f:
                f.write("读取返回json值html出错："+str(albumid)+"\n")
            continue
        count=1
        #print(html)
        for i in html['hotComments']:
            ...
            print("“",albumname,"” ：第 ",count," 条热评","\n")
            print(commentid," / ",commenttime," / ",content," / ",userid," / ",nickname," / ",avturl," / ",likedcount)
            try:
                sql="insert into cmt_album(comment_id,user_id,album_id,comment_content,comment_time,parentCommentId,likedcount) values(%s,%s,%s,%s,%s,%s,%s);"
                cursor.execute(sql,[commentid,userid,albumid,content,commenttime,parentCommentId,likedcount])
                conn.commit()
            except Exception as e:
