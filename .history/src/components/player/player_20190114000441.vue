<template>
    ...
    <audio ref="audio" :src="currentSong.song" @play="ready" @error="error" @timeupdate="updateTime"
           @ended="end"></audio>
...
import lyric from '@/common/js/lyric'
    ...
    export default {
        ...
        methods:{
            ...
            handleLyric() {
                var conp = document.querySelector('.song_info_lyric')
                // 包裹歌词的元素容器
                lyric.lyticsEnd(conp) // 重置对象
                lyric.parse(this.currentSong.lyric, conp) // 解析歌词  this.lyrics 代表歌词数据
                lyric.addcontent(conp) // 向元素容器中添加歌词内容
                lyric.gundong(conp) // 歌词滚动函数
