                            ...
                            <el-table-column width="400px">
                                <template slot-scope="scope">
                                    <div class="icon-menu" style="transform: scale(0.7)">
                                        <a href="javascript:;" title="播放" @click="selectToPlay(scope.row.song_id)">
                                            <i class="icon_button icon_play_opacity"></i>
                                        </a>
                                        <a href="javascript:;" title="添加到歌单" @click="add(scope.row.song_id)">
                                            <el-dropdown>
                                                <span class="el-dropdown-link">
                                                <i class="icon_button icon_add_opacity"></i>
                                                </span>
                                                <el-dropdown-menu slot="dropdown">
                                                    <el-dropdown-item>添加到</el-dropdown-item>
                                                    <el-dropdown-item @click="add(381944418,scope.row.song_id)">为张藜老先生送别</el-dropdown-item>
                                                    <el-dropdown-item @click="add(381944418,scope.row.song_id)">【环球唱片精选】必听百首华语经典</el-dropdown-item>
                                                </el-dropdown-menu>
                                            </el-dropdown>
                                        </a>                
                                    </div>
...
<script>
import {addSongToList} from '@/axios/api'
import IconMenu from '@/base/icon-menu/icon-menu'
import {findSingerById, findSongBySingerId, listAlbum, findSongByAlbum, getLyric, getSongById} from '@/axios/api'
...
export default {
    ...
    methods: {
        ...
        selectToPlay(params) {
            ...
            console.log(params)
            getSongById(params).then(res => {
                if (!res.data)
                    return 
                this.songs = createSong(res.data)
        ...
        add(songlist_id, song_id) {
            let params = {
                songlist_id: songlist_id,
                song_id: song_id
            }
            addSongToList(params).then(res => {
                if (!res) {
                    this.$message({
                        type: error,
                        message: '添加失败'
